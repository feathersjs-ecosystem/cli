...
describe('@feathersjs/cli', () => {
  it('is CommonJS compatible', () => {
    assert.strict.equal(typeof require('../lib'), 'function');
  });

  it('basic functionality', () => {
    assert.strict.equal(typeof cli, 'function', 'It worked');
  });
  ...
  it('runs the program with `generate` argument', function (done) {
    ...
    exec(`node ${path.join(__dirname, '../bin/feathers.js')} generate`, (err, stdout, stderr) => {
      assert.strict.equal(err, null);
      assert.strict.equal(stderr, '');
      assert.ok(
